...
check_wget_curl() {
	...
	if [ -e '/usr/bin/wget' ]; then
		if [ -e '/etc/redhat-release' ]; then
			wget -q https://raw.githubusercontent.com/KetlinUA/hestiacp/release/install/hst-install-rhel.sh -O hst-install-rhel.sh
			if [ "$?" -eq '0' ]; then
			...
			wget -q https://raw.githubusercontent.com/KetlinUA/hestiacp/release/install/hst-install-$type.sh -O hst-install-$type.sh
			if [ "$?" -eq '0' ]; then
	...
	if [ -e '/usr/bin/curl' ]; then
		if [ -e '/etc/redhat-release' ]; then
			curl -s -O https://raw.githubusercontent.com/KetlinUA/hestiacp/release/install/hst-install-rhel.sh
			if [ "$?" -eq '0' ]; then
				bash hst-install-rhel.sh $*
				exit
			else
			...
			curl -s -O https://raw.githubusercontent.com/KetlinUA/hestiacp/release/install/hst-install-$type.sh
			if [ "$?" -eq '0' ]; then
				bash hst-install-$type.sh $*
				exit
			else
