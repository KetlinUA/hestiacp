...
if [ ! -f "$COMPOSER_BIN" ]; then
	...
	if [ $? -ne 0 ]; then
		$BIN/v-add-user-notification admin 'Composer installation failed!' '<p class="u-text-bold">The File Manager will not work without Composer.</p><p>Please try running the installer from a shell session:<br><code>bash $HESTIA/install/deb/filemanager/install-fm.sh</code></p><p>If this issue continues, please <a href="https://github.com/KetlinUA/hestiacp/issues" target="_blank">open an issue on GitHub</a>.</p>'
		fm_error='yes'
	fi
fi
...
if [ "$fm_error" != "yes" ]; then
	...
	if [ -f "/usr/bin/php7.3" ]; then
		...
		if [ $? -ne 0 ]; then
			$BIN/v-add-user-notification admin 'File Manager installation failed!' '<p>Please try running the installer from a shell session:<br><code>bash $HESTIA/install/deb/filemanager/install-fm.sh</code></p><p>If this issue continues, please <a href="https://github.com/KetlinUA/hestiacp/issues" target="_blank">open an issue on GitHub</a>.</p>'
			fm_error="yes"
		fi
	else
